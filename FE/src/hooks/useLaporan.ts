...
export const mapApiLaporanToLaporan = (row: Record<string, unknown>): Laporan => {
  ...
  return {
    ...
    tingkat: ["URGENT", "MENDESAK", "TINGGI", "HIGH", "DARURAT"].includes(String(row.prioritas ?? "").toUpperCase()) ? "MENDESAK" : "STANDAR",
    prioritas: row.prioritas != null ? String(row.prioritas) : "STANDAR",
    foto: fotoUrl,
