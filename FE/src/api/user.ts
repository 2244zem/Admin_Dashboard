...
export async function activateUserToken(id: string) {
  const response = await apiClient.post<ApiResponse<unknown> | unknown>(`/api/admin/user/${id}/activate`, {});
  return unwrapData(response);
}

export async function renewUserToken(id: string, hours: number) {
  const response = await apiClient.post<ApiResponse<unknown> | unknown>(`/api/admin/user/${id}/renew-token`, { hours });
  return unwrapData(response);
}
