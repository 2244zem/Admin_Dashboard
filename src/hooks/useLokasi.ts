...
export function useLokasi() {
  ...
  // Gedung CRUD
  const createGedung = async (payload: { nama: string; alamat: string; kapasitas: string }) => {
    setIsLoading(true);
    try {
      if (!import.meta.env.VITE_API_BASE_URL) {
        await new Promise((resolve) => setTimeout(resolve, 300));
  ...
  const updateGedung = async (id: string, payload: { nama: string; alamat: string; kapasitas: string }) => {
    setIsLoading(true);
    try {
      if (!import.meta.env.VITE_API_BASE_URL) {
        await new Promise((resolve) => setTimeout(resolve, 300));
