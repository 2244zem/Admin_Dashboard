...
function mapApiUserToAppUser(row: any): AppUser {
  ...
  const backendId = String(row.id || row.user_id || "");
  
  // Pass entire row to normalizeRole so it can access role_id
  const mappedRole = normalizeRole(row.role_id || row.role || row);
  
  console.log(`👤 Mapping user: ${name} (${row.email}) → role_id: ${row.role_id} → ${mappedRole}`);
  
  return {
    id: Number.parseInt(backendId.replace(/\D/g, ""), 10) || Date.now(),
    backendId,
    namaLengkap: name,
...
export function useUsers() {
  ...
  const fetchOB = useCallback(async () => {
    ...
    try {
      ...
      // Use /api/admin/user with role_id filter to get ALL OB (active + inactive)
      const OB_ROLE_ID = "62c0a9d8-afd7-45f5-9cb3-6dc6e8a9b8da";
      console.log(`👷 fetchOB: Calling /api/admin/user?role_id=${OB_ROLE_ID}&limit=100`);
      
      const data: any = await apiClient.get(`${ENDPOINTS.USERS_LIST}?role_id=${OB_ROLE_ID}&limit=100`);
      console.log("👷 fetchOB: RAW response from backend:", JSON.stringify(data, null, 2));
      console.log("👷 fetchOB: Response type:", Array.isArray(data) ? "Array" : typeof data);
      
      // Extract users with same logic as fetchUsers
      ...
      if (extracted.length === 0) {
        ...
        console.warn("   1. No OB users exist with role_id =", OB_ROLE_ID);
        console.warn("   2. All OB users are inactive and backend filters them out");
        console.warn("   3. Wrong role_id (check backend database for correct UUID)");
      }
  ...
  const updateUser = async (id: number, payload: any) => {
    ...
    try {
      ...
      if (payload.role) formData.append("role_id", payload.role);
      const res = await apiClient.patch<any>(ENDPOINTS.USERS_UPDATE(String(id)), formData);
      await fetchUsers();
      return res;
    } catch (err: any) {
      const msg = getErrorMessage(err);
  ...
  const deleteUser = async (id: number) => {
    ...
    try {
      ...
      await apiClient.delete(ENDPOINTS.USERS_DELETE(String(id)));
      await fetchUsers();
    } catch (err: any) {
      const msg = getErrorMessage(err);
      setError(msg);
  ...
  const renewToken = async (id: number, hours: number) => {
    setIsLoading(true);
    try {
      if (!import.meta.env.VITE_API_BASE_URL) {
        await new Promise((resolve) => setTimeout(resolve, 300));
      ...
      throw new Error("Endpoint perpanjang token aktivasi belum tersedia di dokumentasi API.");
    } catch (err: any) {
      const msg = getErrorMessage(err);
      setError(msg);
      throw new Error(msg);
