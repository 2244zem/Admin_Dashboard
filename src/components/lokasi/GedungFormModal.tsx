...
export const GedungFormModal: React.FC<GedungFormModalProps> = ({
  ...
  const [form, setForm] = useState<GedungForm>({ nama: "", alamat: "", kapasitas: "" });
  ...
  const handleSimpan = () => {
    if (!form.nama.trim() || !form.kapasitas.trim()) {
      alert("Nama Gedung dan Kapasitas wajib diisi.");
      return;
    }
    onSave(form);
  };
        ...
        >
          ...
          >
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Nama Gedung</label>
                <input
                  type="text"
                  value={form.nama}
                  onChange={(e) => setForm((prev) => ({ ...prev, nama: e.target.value }))}
                  placeholder="Contoh: Gedung A, Menara Utara"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Jl. Sudirman Kav 21"
                  className="w-full bg-white text-gray-700 text-sm rounded-xl px-4 py-2.5 outline-none border border-gray-300 hover:border-gray-400 focus:border-[#0F4C81] focus:ring-2 focus:ring-blue-100 transition-all duration-200"
                />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Kapasitas</label>
                <input
                  type="text"
                  value={form.kapasitas}
                  onChange={(e) => setForm((prev) => ({ ...prev, kapasitas: e.target.value }))}
                  placeholder="Contoh: 5 lantai"
                  className="w-full bg-white text-gray-700 text-sm rounded-xl px-4 py-2.5 outline-none border border-gray-300 hover:border-gray-400 focus:border-[#0F4C81] focus:ring-2 focus:ring-blue-100 transition-all duration-200"
                />
              </div>
            </div>
